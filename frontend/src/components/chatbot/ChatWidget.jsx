...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', content: '¡Hola! Soy el asistente virtual de la **Facultad de Tecnología USFX**. ¿En qué puedo ayudarte? Puedes escribir o usar el micrófono.' }
  ])
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const res = await aiService.chat({ session_id: SESSION_ID, message: text, history })
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          setMessages(prev => [...prev, { role: 'assistant', content: 'No pude transcribir el audio. Por favor escribe tu consulta.' }])
        }
      }
      recorder.start()
  ...
  return (
    <>
      ...
      {/* Chat window */}
      {open && (
        <div className="fixed bottom-24 right-6 w-[350px] max-w-[calc(100vw-24px)] h-[520px] max-h-[80vh] flex flex-col card shadow-2xl z-50 overflow-hidden">
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            {messages.map((msg, i) => (
              <div key={i} className={clsx('flex', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
                <div className={clsx('max-w-[85%] px-3 py-2 text-sm', msg.role === 'user' ? 'chat-user' : 'chat-bot')}>
                  <ReactMarkdown className="prose prose-sm max-w-none prose-p:my-0.5 prose-ul:my-0.5">
                    {msg.content}
                  </ReactMarkdown>
                </div>
              </div>
            ...
            {tramiteLink && (
              <div className="flex justify-start">
                <Link to={`/tramites/${tramiteLink}`} onClick={() => setOpen(false)} className="text-xs bg-blue-50 border border-blue-200 text-blue-700 px-3 py-2 rounded-xl hover:bg-blue-100 transition-colors">
